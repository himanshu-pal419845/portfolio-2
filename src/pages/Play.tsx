...
const Play = () => {
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    { role: 'assistant', content: 'Hello there! I am Himanshu Pal 👋 Ask me anything about my AI or Cybersecurity work!' }
  ]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      console.warn('AI Chat fallback activated in Play.tsx:', error);
      
      // Local smart response fallback helper
      const getPlayMockResponse = (userText: string): string => {
        ...
        if (query.includes("hello") || query.includes("hi ") || query.includes("hey") || query === "hi" || query === "hello") {
          return "Hello! I am Himanshu Pal. Ready for a game of chess? Or do you have questions about my AI/Cybersecurity background? (Or do you just want to hear a bad coding joke?) ♟️";
        }
        ...
        if (query.includes("age") || query.includes("old are you")) {
          return "I am 17 years old. Old enough to build AI agents, too young to rent a car. But hey, I can program my way out of parking tickets (please don't tell the Ghaziabad traffic police!). 🎓";
        }
        if (query.includes("chess") || query.includes("play")) {
          return "I love playing chess! Try making a move on the board to play against my AI opponent! But beware, it's rated ELO 3640. It plays chess much better than it codes, which is both amazing and slightly scary! ♟️";
        }
        
        return "That's interesting! I work with LLMs, python automation, and secure web architectures. Let me know if you want to play a move or ask about my work! 🚀";
      };
      
      await new Promise((resolve) => setTimeout(resolve, 600));
      const mockReply = getPlayMockResponse(chatInput);
      const assistantMessage: ChatMessage = {
        role: 'assistant',
        content: mockReply
      };
